...
def test_stuck_in_a_box_signals_basic(solusdt_1h_data):
    ...
    assert (
        not result["signal"].iloc[5:].isna().any()
    ), "NaN values found in 'signal' column."
    assert (
        not result["exit_signal"].iloc[5:].isna().any()
...
def test_stuck_in_a_box_exit(solusdt_1h_data):
    ...
    # Run exit logic
    result = stuck_in_a_box_exit(df, rsi_overbought=70, near_threshold=0.02)
    ...
    expected_exit = (result["close"] >= result["box_high"] * (1 - 0.02)) | (
        result["rsi"] > 70
    )
    actual_exit = result["exit_signal"] == 1
    assert actual_exit.equals(
        expected_exit.fillna(False)
...
def test_stuck_in_a_box_small_dataframe():
    ...
    small_df = pd.DataFrame(
        {"open": [100, 105], "close": [105, 102], "high": [106, 107], "low": [99, 101]}
    )
...
def test_identify_box(solusdt_1h_data):
    ...
    pd.testing.assert_series_equal(
        result["box_high"], expected_box_high, check_names=False, check_dtype=False
    )
    pd.testing.assert_series_equal(
        result["box_bottom"], expected_box_bottom, check_names=False, check_dtype=False
    )
...
def test_stuck_in_a_box_full_flow(solusdt_1h_data):
    ...
    df = stuck_in_a_box_exit(df, rsi_overbought=70, near_threshold=0.02)
