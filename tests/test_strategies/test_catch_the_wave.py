...
def test_catch_the_wave_signals_basic(btcusdt_1h_data):
    ...
    expected_columns = [
        'ma', 'rsi', 'trend', 'is_hammer',
        'is_engulfing', 'is_doji', 'pullback_zone',
        'rsi_condition', 'signal', 'exit_signal'
    ]
    assert all(col in result.columns for col in expected_columns), "Some expected columns are missing."

    # Assert no NaN values in key columns
    assert not result['signal'].isna().any(), "NaN values found in 'signal' column."
    assert not result['exit_signal'].isna().any(), "NaN values found in 'exit_signal' column."

    # Assert signals are binary (0 or 1)
    assert set(result['signal'].unique()).issubset({0, 1}), "'signal' column contains non-binary values."
    assert set(result['exit_signal'].unique()).issubset({0, 1}), "'exit_signal' column contains non-binary values."
...
def test_catch_the_wave_signals_slope(btcusdt_1h_data):
    ...
    result = catch_the_wave_signals(
        df, ma_period=ma_period, rsi_period=3, rsi_oversold=80, use_ma_slope=True, slope_threshold=0.001
    )
    ...
    expected_columns = [
        'ma', 'rsi', 'trend', 'is_hammer',
        'is_engulfing', 'is_doji', 'pullback_zone',
        'rsi_condition', 'signal', 'exit_signal', 'ma_slope'
    ]
    assert all(col in result.columns for col in expected_columns), "Some expected columns are missing."

    # Assert no NaN values in key columns, excluding the first 'ma_period' rows
    assert not result['signal'].iloc[ma_period:].isna().any(), "NaN values found in 'signal' column."
    assert not result['exit_signal'].iloc[ma_period:].isna().any(), "NaN values found in 'exit_signal' column."
    assert not result['ma_slope'].iloc[ma_period:].isna().any(), "NaN values found in 'ma_slope' column."
...
def test_rsi_conditions_and_signals(btcusdt_1h_data):
    ...
    result = catch_the_wave_signals(df, rsi_period=14, rsi_oversold=30, rsi_overbought=70)
...
def test_catch_the_wave_missing_columns():
    ...
    assert missing_cols == {'open', 'high', 'low'}, f"Expected missing columns {{'open', 'high', 'low'}}, got {missing_cols}"
...
def test_catch_the_wave_rsi_threshold(btcusdt_1h_data):
    ...
    result = catch_the_wave_signals(
        df, ma_period=5, rsi_period=3, rsi_oversold=30, rsi_threshold=50, rsi_overbought=70
    )
    ...
    assert (result['rsi_condition'] == expected_condition).all(), "RSI conditions not applied correctly."
...
def test_catch_the_wave_slope_threshold(btcusdt_1h_data):
    ...
    result = catch_the_wave_signals(
        df, ma_period=5, rsi_period=3, rsi_oversold=80, use_ma_slope=True, slope_threshold=high_slope_threshold
    )
    ...
    expected_trend = (df['close'] > result['ma']) & (result['ma_slope'] > high_slope_threshold)
    assert result['trend'].equals(expected_trend), "Slope threshold not applied correctly."
...
def test_catch_the_wave_exit_missing_rsi():
    ...
    with pytest.raises(ValueError, match="Input DataFrame must contain an 'rsi' column."):
...
def test_catch_the_wave_parameterized(
    btcusdt_1h_data,
    ma_period, rsi_period, rsi_oversold, rsi_overbought,
    use_ma_slope, slope_threshold, expected_min_signals
):
    df = btcusdt_1h_data.copy()
    ...
    buy_signals = (result["signal"] == 1).sum()
    assert buy_signals >= expected_min_signals, f"Expected at least {expected_min_signals} buy signals; got {buy_signals}"
