...
class StuckInABoxStrategy(bt.Strategy):
    ...
    def __init__(self):
        # Rolling max/min
        self.box_top = bt.indicators.Highest(
            self.data.high, period=self.params.box_period
        )
        self.box_bottom = bt.indicators.Lowest(
            self.data.low, period=self.params.box_period
        )
    ...
    def next(self):
        near_bottom = self.data.close[0] <= self.box_bottom[0] * (
            1 + self.params.near_threshold
        )
        ...
        near_top = self.data.close[0] >= self.box_top[0] * (
            1 - self.params.near_threshold
        )
